...
export default function MetricsTable({ config }: { config: TableConfig }) {
  ...
  if (err) return <div className="mtable"><div className="gridwrap" style={{ padding: 20, color: "var(--t-tx-2)", fontSize: 12 }}>Não foi possível carregar: {err}</div></div>;
              ...
              {COLS.map((c) => {
                ...
                return (
                  <th key={c.key} className={cls}>
                    ...
                    {c.key === "vol30" && <span className="scale" style={{ color: "var(--t-tx-3)" }}>{c.unit}</span>}
                    {c.key === "stu" && (cov < total ? <span className="scale warn">{cov}/{total}</span> : <span className="scale" style={{ color: "var(--t-tx-3)" }}>{c.unit}</span>)}
                  </th>
                );
              })}
            </tr>
...
function Cell({ col, rd, downgraded, onOpen, isOpen }: { col: Col; rd: RowData; downgraded: boolean; onOpen: (r: DOMRect) => void; isOpen: boolean }) {
  ...
  if (col.type === "num" || (col.type === "bar" && downgraded)) {
    ...
    return <td className="num" style={naCell ? { color: "var(--t-tx-3)" } : undefined}>{txt}</td>;
  }

  if (col.type === "numchg") {
    if (naCell) return <td className="num" style={{ color: "var(--t-tx-3)" }}>—</td>;
