...
/**
 * Card de ESTRUTURA A TERMO: X = vencimento, Y = settlement; a linha liga os
 * contratos. Contango (sobe) = futuro mais caro que a vista (carrego /
 * expectativa de alta); backwardation (desce) = vista mais cara (escassez
 * atual). FATO de mercado com a explicacao, NUNCA recomendacao. Fonte B3/brapi.
 */
function FuturesCurveCard({ curve }: { curve: FuturesCurve }) {
  ...
  const W = 244, H = 96, ml = 4, mr = 6, mt = 16, mb = 16;
      ...
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full">
        ...
        {pts.map((p, i) => (
          <g key={p.symbol}>
            ...
            {(i === 0 || i === pts.length - 1) && (
              <text x={x(i)} y={i === 0 ? y(p.settlement) - 4 : y(p.settlement) - 4} textAnchor={i === 0 ? "start" : "end"} style={{ fontFamily: "monospace", fontSize: "6px", fill: "rgba(255,255,255,0.65)" }}>
